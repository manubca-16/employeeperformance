...
const { buildActivityVisibilityFilter } = require("../utils/activity");
...
exports.list = async (req, res, next) => {
  try {
    ...
    const visibilityFilter = buildActivityVisibilityFilter(req.user);
    const activities = await Activity.find(visibilityFilter)
      .sort({ createdAt: -1 })
      .skip(skip)
      .limit(limit);
    res.json(activities);
